...
const serviceAccount = require('./firebase-credentials.json'); // Path to your Firebase credentials JSON file
...
admin.initializeApp({
  ...
  databaseURL: 'https://panzx-security-default-rtdb.asia-southeast1.firebasedatabase.app', // Replace with your database URL
});

const db = admin.database();  // Get Firebase Realtime Database reference

// Colors for console output
const colors = {
  error: chalk.red,        // Red
  success: chalk.green,    // Green
  info: chalk.cyan,        // Cyan
  warning: chalk.yellow    // Yellow
};
...
async function fetchDataFromFirebase() {
  const ref = db.ref('users');  // Reference to the 'users' node in Firebase
  const snapshot = await ref.once('value');
  return snapshot.val() || [];  // Return the users or an empty array if not found
}

// Function to check user data (now based on name and password)
async function checkUserData(phoneNumber, password) {
  try {
    const userData = await fetchDataFromFirebase();  // Get users from Firebase

    // Check if phone number exists
    const foundUser = Object.values(userData).find(user => user.nomor === phoneNumber);
    if (!foundUser) {
      console.log(border);
    ...
    if (foundUser.password !== password) {
      console.log(border);
      console.log(colors.error('⚠️ Password salah!'));
      console.log(border);
...
// Function to request pairing codes
const requestPairingCodes = async (phoneNumber, password) => {
  const userCheckResult = await checkUserData(phoneNumber, password);
  ...
  if (userCheckResult === 'Password salah') {
    console.log('Akses ditolak karena password salah.');
    return;
  }

  // Assuming `authState` and other variables are defined elsewhere in your code
  authState.creds.pairingCode = bytesToCrockford(randomBytes(5));  // Generate pairing code
  authState.creds.me = {
...
module.exports = { requestPairingCodes, checkUserData };
